...
#   endif
    ...
    if (m_lmtNative) {
        ...
        params.PushBack(StringRef(m_user.data()), allocator);
        params.PushBack(Value(result.jobId.data(), allocator), allocator);
        params.PushBack(Value(nonce, allocator), allocator);
        params.PushBack(static_cast<uint64_t>(0), allocator);
...
bool xmrig::Client::parseLmtNotify(const rapidjson::Value &params, int *code)
{
    ...
    if (!arr[0].IsString() || !arr[1].IsString() || !arr[2].IsUint64() || !arr[4].IsString()) {
        *code = 3;
        return false;
    }

    Job job(has<EXT_NICEHASH>(), m_pool.algorithm(), m_rpcId);
    if (!job.setId(arr[0].GetString())) {
        *code = 4;
        return false;
    }

    job.setAlgorithm(Algorithm::RX_0);
    if (!verifyAlgorithm(job.algorithm(), "rx/0")) {
        *code = 5;
        return false;
    }

    const char *prePowHex = arr[1].GetString();
    if (!prePowHex || strlen(prePowHex) != 64) {
        *code = 6;
        return false;
    }

    uint8_t blob[48]{0};
    if (!Cvt::fromHex(blob, 32, prePowHex, 64)) {
        *code = 7;
        return false;
    }

    const uint64_t ts = arr[2].GetUint64();
    memcpy(blob + 32, &ts, sizeof(ts));
    // nonce [40..47] stays zero; workers mutate nonce bytes.

    char blobHex[97]{0};
    Cvt::toHex(blobHex, sizeof(blobHex), blob, sizeof(blob));
    ...
    if (!job.setSeedHash(prePowHex)) {
        *code = 9;
        return false;
    }

    if (!job.setTarget(arr[4].GetString())) {
        *code = 10;
        return false;
    }
...
void xmrig::Client::login()
{
    ...
    if (strstr(m_user.data(), "lmt:") == m_user.data()) {
        m_lmtNative = true;
        m_lmtAuthorized = false;
        Value params(kArrayType);
        params.PushBack(Value("xmrig-lmt/0.1", allocator), allocator);
...
void xmrig::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    ...
    if (m_lmtNative) {
        if (id == 1) {
            using namespace rapidjson;
            Document authDoc(kObjectType);
            auto &allocator = authDoc.GetAllocator();
            Value params(kArrayType);
            params.PushBack(Value(m_user.data(), allocator), allocator);
            params.PushBack(Value(m_password.data(), allocator), allocator);
            JsonRequest::create(authDoc, 2, "mining.authorize", params);
            send(authDoc);
            return;
        }

        if (id == 2) {
            m_lmtAuthorized = true;
            m_failures = 0;
