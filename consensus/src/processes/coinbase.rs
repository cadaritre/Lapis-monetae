...
// We define a target emission horizon of 8 years. The pre-deflationary period lasts ~0.5 years.
// To adapt the deflationary schedule (426 steps) to the remaining ~7.5 years, we compress the
// effective "month" length accordingly (~6.45 days per step).
// 7.5 years in seconds = 7.5 * 365.25 * 24 * 60 * 60 = 236,682,000 seconds.
// Adding back the early 3-day network downtime adjustment leads to 236,941,200 seconds.
// SECONDS_PER_MONTH = 236,941,200 / 426 ≈ 556,920 seconds.
const SECONDS_PER_MONTH: u64 = 556_920;
...
impl CoinbaseManager {
    pub fn new(
        ...
        let pre_deflation_total: u128 = (pre_deflationary_phase_base_subsidy as u128)
            * (deflationary_phase_daa_score as u128);

        // Deflationary minted amount using the compressed SECONDS_PER_MONTH (BPS cancels out)
        let deflation_total: u128 = SUBSIDY_BY_MONTH_TABLE
            .iter()
            .map(|v| (*v as u128) * (SECONDS_PER_MONTH as u128))
            .sum();

        // Total scheduled without scaling
        ...
        Self {
            ...
            pre_deflationary_phase_base_subsidy: scaled_pre_deflationary_base,
            bps,
...
mod tests {
    ...
    fn payload_serialization_test() {
        ...
        let expected_extra: [u8; 7] = [48u8, 46, 49, 50, 46, 56, 47];
    ...
    fn modify_payload_test() {
        ...
        let extra_data2 = [2u8, 3, 23, 98, 34, 34];
