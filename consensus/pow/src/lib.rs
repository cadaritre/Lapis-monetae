...
impl State {
    ...
    pub fn new(header: &Header) -> Self {
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            Self { target, pre_pow_hash, timestamp: header.timestamp, matrix, vm }
        }
    ...
    pub fn from_pre_pow(pre_pow_hash: Hash, timestamp: u64, bits: u32) -> Self {
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            Self { target, pre_pow_hash, timestamp, matrix, vm }
        }
        #[cfg(target_arch = "wasm32")]
        {
            let target = Uint256::from_compact_target_bits(bits);
    ...
    pub fn calculate_pow(&self, nonce: u64) -> Uint256 {
        ...
        {
            // Native PoW pipeline:
            // pre_pow_hash + timestamp + nonce -> cSHAKE PoW hash -> RandomX -> kHeavyHash(matrix)
            let pow_hash = PowHash::new(self.pre_pow_hash, self.timestamp).finalize_with_nonce(nonce);
            let randomx_bytes = self.vm.calculate_hash(&pow_hash.as_bytes()).expect("RandomX PoW hash calculation failed");
            let randomx_hash = Hash::from_slice(&randomx_bytes);
            let hash = self.matrix.heavy_hash(randomx_hash);
