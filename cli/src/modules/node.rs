...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("lmtd").expect("Failed to create node settings store"),
...
impl Handler for Node {
    ...
    fn help(&self, _ctx: &Arc<dyn Context>) -> &'static str {
        "Manage the local Lapis Monetae (LMT) node instance"
    }
    ...
    async fn main(self: Arc<Self>, ctx: Arc<KaspaCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        ...
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting LMT node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting LMT node... {}", style("(use 'node mute' to mute logging)").dim());
                }
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<KaspaCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                ("restart", "Restart the local Lapis Monetae (LMT) node instance"),
                ("kill", "Kill the local Lapis Monetae (LMT) node instance"),
                ("status", "Get the status of the local Lapis Monetae (LMT) node instance"),
